...
static void process_input(void)
{
	...
	/* 余白のクリックでキャンセル可能な場合 */
	if (is_click_cancel && pointed_index == -1 && is_left_clicked) {
		/* フェードアウトか終了処理を行う */
		if (fade_out_time > 0) {
			is_fading_out = true;
			reset_lap_timer(&fade_sw);
			       ...
			       const char *text, bool multiline)
{
	...
	color = make_pixel(0xff,
			   ...
			   (uint32_t)conf_menu_save_font_b);
	outline_color = make_pixel(0xff,
				   (uint32_t)conf_menu_save_font_outline_r,
	...
	construct_draw_msg_context(
		...
		text,
		conf_menu_save_font,
		conf_menu_save_font_size,
		conf_menu_save_font_size,
		conf_menu_save_font_ruby,
		conf_menu_save_font_outline,
		x,
		y,
		b->width,
		b->height,
...
static void draw_history_text_item(int button_index)
{
	...
	color = make_pixel(0xff,
			   ...
			   (pixel_t)conf_menu_history_font_b);
	outline_color = make_pixel(0xff,
				   (pixel_t)conf_menu_history_font_outline_r,
	...
	if (!conf_menu_history_font_tategaki) {
		...
		pen_x = b->width - b->margin - conf_menu_history_font_size;
		pen_y = b->margin;
	}
	...
	construct_draw_msg_context(
		...
		-1,		/* Not for a layer: use an alternative image. */
		text,
		conf_menu_history_font,
		conf_menu_history_font_size,
		conf_menu_history_font_size,
		conf_menu_history_font_ruby,
		conf_menu_history_font_outline,
		pen_x,
		pen_y,
		b->width,	/* area_width */
		...
		true,		/* ignore_wait */
		NULL,		/* inline_wait_hook */
		conf_menu_history_font_tategaki);
	set_alternative_target_image(&context, b->rt.img);
	total_chars = count_chars_common(&context, NULL);
	draw_msg_common(&context, total_chars);
}
...
static int get_wait_time(int index)
{
	...
	return (int)((float)button[index].rt.total_chars * AUTO_MODE_TEXT_WAIT_SCALE * get_auto_speed() * 1000.0f);
}
