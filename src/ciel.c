...
/*
 * The Ciel Direction System (By Asatsuki and ktabata)
 */
...
static bool init_cl_pos(bool *cont)
{
	...
	if (!IS_EMPTY(align)) {
		if (strcmp(align, "left") == 0) {
			...
			ts.x[index] = conf_game_width - w;
		} else if (strcmp(align, "center") == 0) {
			ts.x[index] = (conf_game_width - w) / 2;
		} else {
	...
	if (!IS_EMPTY(valign)) {
		if (strcmp(valign, "top") == 0) {
			...
			ts.y[index] = conf_game_height - h;
		} else if (strcmp(align, "center") == 0) {
			ts.y[index] = (conf_game_height - h) / 2;
		} else {
