...
static bool init(void)
{
	...
	/* パラメータを取得する */
	pos_s = get_string_param(MOVE_PARAM_POS);
	span = get_float_param(MOVE_PARAM_SPAN);
	accel_s = get_string_param(MOVE_PARAM_ACCEL);
	ofs_x = get_int_param(MOVE_PARAM_OFFSET_X);
	ofs_y = get_int_param(MOVE_PARAM_OFFSET_Y);
	alpha_s = get_string_param(MOVE_PARAM_ALPHA);
