		      ...
		      pixel_t name_color, pixel_t name_outline_color)
{
	...
	/* 引用符を取得する */
	quote_prefix = conf_menu_history_quote_prefix;
	quote_start = conf_menu_history_quote_start;
	quote_end = conf_menu_history_quote_end;
	if (quote_prefix == NULL)
		quote_prefix = "";
	if (quote_start == NULL)
		quote_start = compare_locale("ja") ? U8("「") : U8(" \"");
	if (quote_end == NULL)
		quote_end = compare_locale("ja") ? U8("」") : U8("\"");
