...
static bool get_position(const char *pos, struct image *img, int ofs_x, int ofs_y, int *chpos, int *xpos, int *ypos);
	...
	*xpos = 0;
	...
	if (strcmp(pos, "back") == 0 ||
		...
		*chpos = CH_RIGHT_CENTER;
		if (img != NULL) {
			int center = (conf_game_width - img->width) / 2 + ofs_x;
			int right = conf_game_width - img->width - conf_stage_ch_margin_right + ofs_x;
