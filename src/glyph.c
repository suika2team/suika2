...
bool init_glyph(void)
{
	...
	for (i = 0; i < FONT_COUNT; i++) {
		...
		err = FT_New_Memory_Face(library,
					 ...
					 &face[i]);
		if (err != 0) {
			log_font_file_error(conf_font_ttf[i]);
			return false;
		}
	}
	...
	/* フォントのプリロードを行う */
	for (i = 0; i < FONT_COUNT; i++) 
		get_glyph_width(i, conf_msgbox_font_size, 'A');

	/* エモーティコンのロードを行う */
	for (i = 0; i < EMOTICON_COUNT; i++) {
		if (conf_emoticon_name[i] != NULL && conf_emoticon_image[i] != NULL) {
			emoticon_image[i] = create_image_from_file(CG_DIR, conf_emoticon_image[i]);
	...
	bool use_tategaki)
{
	...
	context->bg_color = make_pixel((uint32_t)255, (uint32_t)0, (uint32_t)0, (uint32_t)0);
	...
	/* Is quoted? */
	if (is_quoted_serif(msg) && !use_tategaki)
