...
/*
 * The Audio HAL for Unity.
 */

using UnityEngine;

[RequireComponent(typeof(AudioSource))]
public class OpenNovelAudio : MonoBehaviour
{
	unsafe byte *wave;
	...
	unsafe void OnAudioFilterRead(float[] data, int channels)
	{
		...
		{
			OpenNovelScript.get_wave_samples(wave, (uint *)unsafePointer, samples);
			if (channels == 2)
			{
		...
		if (OpenNovelScript.is_wave_eos(wave))
			wave = null;
	}
}
