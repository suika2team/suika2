...
// conf_game_heightris Engine Base
#import "opennovel.h"
#import "pro.h"
#import "package.h"
...
- (BOOL)createProject:(NSString *)templateName {
    ...
    FILE *fp = fopen([[[[panel URL] path] stringByAppendingString:@"/game.opennovel"] UTF8String], "w");
    if (fp == NULL) {
        NSAlert *alert;
...
- (BOOL)copyResourceTemplate:(NSString *)from {
    NSArray *subfolderArray = @[@"/font", @"/image", @"/script", @"/sound", @"/story", @"/video", @"/project.txt"];
...
- (BOOL)openProject {
    while (YES) {
        ...
        if ([[path lastPathComponent] hasSuffix:@"game.opennovel"]) {
            NSAlert *alert = [[NSAlert alloc] init];
            [alert setMessageText:_isEnglish ?
        ...
        if (![fileManager isReadableFileAtPath:[path stringByAppendingString:@"/project.txt"]]) {
            NSAlert *alert = [[NSAlert alloc] init];
            [alert setMessageText:_isEnglish ?
...
- (CGPoint)windowPointToScreenPoint:(CGPoint)windowPoint {
    ...
    return CGPointMake(x, conf_game_height - y);
}
...
- (IBAction)onCreateJapaneseADV:(id)sender {
    if ([self createProject:@"game"])
        [self setupView];
}

- (IBAction)onCreateJapaneseNVL:(id)sender {
//    if ([self createProject:@"japanese-nvl"])
//        [self setupView];
}

- (IBAction)onCreateJapaneseNVLVertical:(id)sender {
//    if ([self createProject:@"japanese-nvl-vertical"])
//        [self setupView];
}

- (IBAction)onCreateEnglishADV:(id)sender {
//    if ([self createProject:@"english-adv"])
//        [self setupView];
}

- (IBAction)onCreateEnglishNVL:(id)sender {
//    if ([self createProject:@"english-nvl"])
//        [self setupView];
}
...
- (IBAction)onMenuExportForMac:(id)sender {
    ...
    if([alert runModal] != NSAlertFirstButtonReturn) {
        ...
        if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/package.pak", [fileManager currentDirectoryPath]]
                                  toPath:[NSString stringWithFormat:@"%@/Resources/package.pak", exportPath]
                                   error:nil]) {
        ...
        if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/package.pak", [fileManager currentDirectoryPath]]
                                  toPath:[NSString stringWithFormat:@"%@/package.pak", exportPath]
                                   error:nil]) {
            log_warn("Copy error (2).");
            return;
        }
...
- (IBAction)onMenuExportForPC:(id)sender {
    ...
    if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/package.pak", [fileManager currentDirectoryPath]] toPath:[NSString stringWithFormat:@"%@/export-pc/package.pak", [fileManager currentDirectoryPath]] error:nil]) {
        log_warn("Copy error (1).");
        return;
    }
...
- (IBAction)onMenuExportForWeb:(id)sender {
    ...
    for (NSString *sub in appArray) {
        if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/Contents/Resources/html/%@", [[NSBundle  mainBundle] bundlePath], sub]
                                  toPath:[NSString stringWithFormat:@"%@/%@", exportPath, sub]
                                   error:nil]) {
            log_warn("Copy error (2).");
            return;
    ...
    if (![fileManager createDirectoryAtURL:[NSURL fileURLWithPath:[exportPath stringByAppendingString:@"/video"]] withIntermediateDirectories:YES attributes:nil error:nil]) {
        log_warn("mkdir error (2).");
        return;
    }

    NSArray *contents = [fileManager contentsOfDirectoryAtPath:[NSString stringWithFormat:@"%@/video", [fileManager currentDirectoryPath]] error:nil];
    for (NSString *item in contents) {
        if ([[item pathExtension] isEqualToString:@"mp4"]) {
            if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/video/%@", [fileManager currentDirectoryPath], item]
                                      toPath:[NSString stringWithFormat:@"%@/video/%@", exportPath, item]
                                       error:nil]) {
...
- (IBAction)onMenuExportForIOS:(id)sender {
    ...
    NSArray *appArray = @[@"build.sh", @"engine-ios", @"engine-ios.xcodeproj", @"ExportOptions.plist", @"libroot-device", @"libroot-sim", @"Resources"];
    for (NSString *sub in appArray) {
...
- (IBAction)onMenuExportForAndroid:(id)sender {
    ...
    NSArray *appArray = @[@"app", @"build.bat", @"build.gradle", @"gradle", @"gradle.properties", @"gradlew", @"gradlew.bat", @"settings.gradle"];
    for (NSString *sub in appArray) {
    ...
    NSArray *subfolderArray = @[@"font", @"image", @"script", @"sound", @"story", @"project.txt"];
    for (NSString *sub in subfolderArray) {
    ...
    if (![fileManager createDirectoryAtURL:[NSURL fileURLWithPath:[exportPath stringByAppendingString:@"/app/src/main/assets/video"]] withIntermediateDirectories:YES attributes:nil error:nil]) {
        log_warn("mkdir error (2).");
        return;
    }
    ...
    for (NSString *item in contents) {
        if ([[item pathExtension] isEqualToString:@"mp4"]) {
            if (![fileManager copyItemAtPath:[NSString stringWithFormat:@"%@/video/%@", [fileManager currentDirectoryPath], item]
                                      toPath:[NSString stringWithFormat:@"%@/app/src/main/assets/video/%@", exportPath, item]
                                       error:nil]) {
                log_warn("Copy error (3).");
                return;
            }
...
- (IBAction)onMenuExportForUnity:(id)sender {
    ...
    NSArray *appArray = @[@"Assets", @"README.md"];
    for (NSString *sub in appArray) {
...
- (IBAction)onHelp:(id)sender {
    if (_isEnglish) {
        [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"https://conf_game_heightris Engine.com/en/wiki/"]];
    } else {
        [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"https://conf_game_heightris Engine.com/wiki/"]];
    }
}
    ...
    for (int i = 0; i < lineArray.count; i++) {
        ...
        // コマンド行を処理する
            ...
            if (commandType != COMMAND_SET && commandType != COMMAND_IF &&
                commandType != COMMAND_UNLESS && commandType != COMMAND_TEXT &&
