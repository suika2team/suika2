...
/*
 * Dialog procedure for the @menu command.
 */
...
static BOOL OnFinish(HWND hWnd)
{
	...
	if (wcscmp(wszText, L"") == 0)
	{
		MessageBox(hWnd,
				   bEnglish ?
				   L"Fill the menu file name." :
				   L"メニューファイル名を指定してください。",
				   DIALOG_TITLE,
...
BOOL CALLBACK DlgMenuWndProc(HWND hWnd, UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	UINT nID;
